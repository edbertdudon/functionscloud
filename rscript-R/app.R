  ...
  for (s in 1:length(slidesNoRewrite)) {
    longitude <- unlist(slidesNoRewrite[s])
		...
		if (is.null(longitude)) {
			longitude <- matrix(,nrow=0,ncol=0)
		} else {
...
getErrorType <- function(cell) {
  # for (sheet in dataNamesNoRewrite) {
  #   # exp(test.csv[2:10,1]) applicable to circularReference when it shouldn't.
  #   #Returns more than one result when we don't want it to.
...
createLattitudeFromRange <- function(range) {
  ...
  if (currentSlide == range) return(range)
...
#' @post /plot
#' @png
function(slides, names, range, firstrow, types, variablex=NA, variabley) {
	if (is.na(variablex)) {
    pngChart <- ggplot() + geom_blank()
    return(print(pngChart))
  }
...
summaryStatistics <- function(x, na.omit=TRUE) {
  if (na.omit)
    x <- x[!is.na(x)]
  m <- mean(x)
  n <- length(x)
...
addRownamesColnames <- function(lattitude) {
  lattitude %>%
    rownamesToCol() %>%
    colnamesToRow()
}
...
function(slides, names, range, firstrow, variablesx) {
  ...
  variablesx <- fromJSON(variablesx)
...
#' @post /onewaytable
function(slides, names, range, firstrow, variablex) {
  slidesNoRewrite <<- fromJSON(slides, simplifyMatrix=F)
  dataNamesNoRewrite <<- fromJSON(names)
  currentLattitude <<- createLattitudeFromRange(range)
  currentLattitude <<- lattitudeAsDataframe(firstrow, currentLattitude, range)

  variablex <- currentLattitude[,variablex]

  ftable(variablex) %>%
    as_tibble() %>%
    rename(variablex=Var1, n=Freq) %>%
    toJSON(digits=NA)
}
...
function(slides, names, range, firstrow, variablex, variabley) {
  ...
  regress <- ftable(currentLattitude[c(variablex, variabley)]) %>%
    as.matrix() %>%
    addRownamesColnames()
  regress[1,1] <- paste0(c(variablex, variabley), collapse='_')

  toJSON(regress, digits=NA)
}
...
function(slides, names, range, firstrow, variablex, variabley) {
  ...
  variablex <- currentLattitude[,variablex]
  variabley <- currentLattitude[,variabley]

  chisq.test(variablex, variabley) %>%
    tidy() %>%
    select(-method) %>%
    mutate(feature=variablex, label=variabley) %>%
    rename(degrees_of_freedom=parameter, p_value=p.value) %>%
    relocate(feature, label, p_value, degrees_of_freedom) %>%
    toJSON(digits=NA)
}
...
function(slides, names, range, firstrow, variablex, variabley, variablez, alternative='t', confidencelevel=0.95) {
  ...
  variablex <- currentLattitude[,variablex]
  variabley <- currentLattitude[,variabley]
  variablez <- currentLattitude[,variablez]
...
function(slides, names, range, firstrow, variablesx, variablesy, method='p') {
  ...
  variablesx <- fromJSON(variablesx)
  variablesy <- fromJSON(variablesy)

  cov(
    currentLattitude[,variablesx],
    currentLattitude[,variablesy],
    method=method
  ) %>% addRownamesColnames()
  regress[1,1] <- 'term'
...
function(slides, names, range, firstrow, variablex, groups) {
  ...
  variablex <- currentLattitude[,variablex]
  groups <- currentLattitude[,groups]
...
function(slides, names, range, firstrow, variablex, groups, blocks) {
  ...
  variablex <- currentLattitude[,variablex]
  groups <- currentLattitude[,groups]
  blocks <- currentLattitude[,blocks]
...
function(slides, names, range, firstrow, variablex, variablesy) {
  ...
  paste0(
    'cbind(',
    paste(paste0('`', variablesy, '`'), collapse=','),
    ')~`', variablex, '`'
  ) %>% jsonTidyManova()
}
...
#' @post /optimization
function(
  slides, names, objective=NA, quadratic=NA, linear=NA, gradient=NA, hessian=NA, minmax, decision=NA,
  flhs=NA, fdir=NA, frhs=NA, blhs=NA, bdir=NA, brhs=NA, lowerindex=NA, lowerbound=NA, upperindex=NA,
  upperbound=NA, lowerlimit=NA, upperlimit=NA, jacobian=NA, qquad=NA, qlin=NA, qdir=NA, qrhs=NA, llin=NA,
  ldir=NA, lrhs=NA, c0lhs=NA, c0cone=1, c0rhs=NA, cllhs=NA, lcone=1, clrhs=NA, csolhs=NA, socone=1,
  csorhs=NA, cexlhs=NA, excone=1, cexrhs=NA, cpplhs=NA, ppcone=0.5, cpprhs=NA, cpdlhs=NA, pdcone=0.5,
  cpdrhs=NA, cpsdlhs=NA, psdcone=1, cpsdrhs=NA, solver
) {
  slidesNoRewrite <<- fromJSON(slides, simplifyMatrix=F)
	dataNamesNoRewrite <<- fromJSON(names)
  configureSheetsToMatrix()
  ...
  if (!is.na(blhs) && !is.na(bdir) && !is.na(brhs) && !is.na(objective)) {
  ...
  # Bounds lb, ub, ld, ud
  if (!is.na(lowerbound) || !is.na(upperbound) || !is.na(lowerlimit) || !is.na(upperlimit)) {
    if (is.na(lowerindex)) {
      lowerindex <- NULL
    } else {
