...
library(googleAuthR)
gar_auth_service("/tmp/tart-90ca2-081a368d40ef.json", scope = c("https://www.googleapis.com/auth/cloud-platform", "https://www.googleapis.com/auth/cloudkms"))
...
authUser <- strsplit(substr(args[[1]], 6, nchar(args[[1]])), "\\/")[[1]][1]
...
listToDataframe <- function(data) {
  df <- as.data.frame(matrix(NA, (length(data) - 1), length(data[[1]])))
...
translateRForProccess <- function(cell, currentSlide) {
	...
	if (length(match) == 0) return(removedEqualsCell)

	for (reference in 1:length(match)) {
		column <- lettersToColumn(regmatches(match[reference], regexpr("[[:upper:]]{,2}", match[reference])))
		# column <-  match(tolower(regmatches(match[reference], regexpr("[[:upper:]]{,2}", match[reference]))), LETTERS_REFERENCE)
		...
		if (!identical(reference+1,length(match)-1)) {
			...
			column2 <- lettersToColumn(regmatches(match[reference+1], regexpr("[[:upper:]]{,2}", match[reference+1])))
			row2 <- as.numeric(regmatches(match[reference+1], regexpr("\\d+", match[reference+1])))-1

			if (!grepl("\\d", match[reference])) {
				ref5 <- paste0("[,", column, ":", column2, "]")
				prefix5 <- paste0("!", match[reference], ":", match[reference+1])
				removedEqualsCell <- gsub(prefix5, ref5, removedEqualsCell)

				ref6 <- paste0("`", sheetNamesNoRewrite[currentSlide], "`", "[,", column, ":", column2, "]")
				prefix6 <- paste0(match[reference], ":", match[reference+1])
				removedEqualsCell <- gsub(prefix6, ref6, removedEqualsCell)
			}

			ref4 <- paste0("[", row, ":", row2, ",", column, ":", column2, "]")
			prefix4 <- paste0("!", match[reference], ":", match[reference+1])
			...
			prefix3 <- paste0(match[reference], ":", match[reference+1])
...
splitAndReplaceNoRewrite <- function(cell) {
  if (is.null(cell) || is.na(cell) || cell == "" || is.numeric(cell)) return(cell)

  stringParts <- strsplit(cell, "\\+|\\-|\\*|\\/|\\^|\\%%|\\%/%|\\(|\\)")[[1]]
  evaluateParts <- simplify2array(lapply(stringParts, evaluateCellNoRewrite))
  newCell <- cell
...
computeSparklyR <- function(cell) {
  ...
  for (name in 1:length(sheetNamesNoRewrite)) {
    ...
    proto_name <- regmatches(cell, regexpr(sheetNamesNoRewrite[name], cell))
    proto_cell <- gsub(sheetNamesNoRewrite[name], "e", cell)

    # Break loop when sheet name is found
    if (length(proto_name) != 0) break
  }
...
substituteMatrix <- function(cell) {
	if (is.null(cell) || is.na(cell) || cell == "" || is.numeric(cell) || !grepl(":", cell)) return(cell)

	proto_matrix <- sub("\\[{1}\\d+\\:?\\d*\\,{1}\\d+\\:?\\d*\\]{1}", "", cell)
...
splitAndReplaceMatrix <- function(cell) {
	...
	if (length(matrixParts) > 0) {
		matrixParts <- simplify2array(lapply(matrixParts, substituteMatrix))
	}

	for (proto_matrix in matrixParts) {
		if (exists(proto_matrix)) {
			currentLattitude <- eval(parse(text = proto_matrix))
			currentLattitude <- as.data.frame(currentLattitude, stringsAsFactors = FALSE)
			currentLattitudeColumns <- colnames(currentLattitude)
			currentLattitude <- as.data.frame(lapply(currentLattitude, function(x) as.numeric(as.character(x))), stringsAsFactors = FALSE)
			colnames(currentLattitude) <- currentLattitudeColumns
			eval(call("<-", as.name(proto_matrix), currentLattitude))
		}
	}

	result <- tryCatch({eval(parse(text=cell))}, error = function(cond) return('#ERROR!'))
	return(result)
}
...
compareMissingValues <- function(currentLattitude, currentSlide) {
	for (row in 1:nrow(currentLattitude)) {
		for (column in 1:ncol(currentLattitude)) {
			...
			if (is.null(rawCell) || is.na(rawCell) || rawCell == "" || !grepl('#ERROR!', cell)) next

			storedString <- splitAndReplaceNoRewrite(evaluateCellNoRewrite(rawCell))
			storedString <- as.numeric(as.character(splitAndReplaceMatrix(storedString)))
...
saveBackToJsonFromDataFrame <- function(dataframe, json) {
  for (row in 1:(length(json)-1)) {
    for (column in 1:length(json[[1]])) {
      if (!is.null(json[[row+1]][[column]]$value) && startsWith(json[[row+1]][[column]]$value, "=")) {
...
plot <- function(chartdata) {
  ...
  currentLattitude <- as.data.frame(eval(parse(text = paste0("`", name, "`"))), stringsAsFactors = FALSE)
  currentLattitudeColumns <- colnames(currentLattitude)
  currentLattitude <- as.data.frame(lapply(currentLattitude, function(x) as.numeric(as.character(x))), stringsAsFactors = FALSE)
  colnames(currentLattitude) <- currentLattitudeColumns
  ...
  return(base64enc::dataURI(file="/tmp/ggplot.png", mime="image/jpeg"))
}

library(broom)
regressionNormal <- function(name, formulatext, statistictext) {
  if (exists(gsub(" ", "_", name))) {
    name <- gsub(" ", "_", name)
  }
  ...
  completeFormulaText <- paste("lm(", formulatext, ", currentLattitude) %>%", statistictext)
  currentLattitude <- eval(parse(text = completeFormulaText)) %>% tidy()
  return(currentLattitude)
}

regressionSparklyR <- function(name, formulatext, statistictext) {
  if (exists(gsub(" ", "_", name))) {
    name <- gsub(" ", "_", name)
  }
  statistictext <- substring(statistictext, 1, nchar(statistictext)-1)

  completeFormulaText <- paste0("`", name, "`%>%spark_apply(function(e)broom::tidy(", statistictext, "lm(", formulatext, ",e))))")
  pickup_dropoff_tbl <- tryCatch({
    eval(parse(text = completeFormulaText))
  }, error = function(cond) return('#ERROR!'))
...
regression <- function(regressiondata) {
  ...
  statistictext <- regressiondata$statistictext
  ...
  for (name in 1:length(sheetNamesNoRewrite)) {
    if (identical(initialDataNoRewrite[[name]]$name, regressionSheetname)) {
      if (identical(initialDataNoRewrite[[name]]$type, "input")) {
        regressionParsed <- regressionSparklyR(regressionSheetname, formulatext, statistictext)
        return(regressionParsed)
      } else {
        regressionParsed <- regressionNormal(regressionSheetname, formulatext, statistictext)
        return(regressionParsed)
      }
      break
    }
...
for (slide in 1:length(initialDataNoRewrite)) {
	if (identical(initialDataNoRewrite[[slide]]$type, "sheet")) {
		currentSlide <- eval(parse(text = initialDataNoRewrite[[slide]]$name))
		longitudeTemporaryData <- apply(currentSlide, 1:2, splitAndReplace)
		eval(call("<-", as.name(initialDataNoRewrite[[slide]]$name), longitudeTemporaryData))
	}
}
...
for (slide in 1:length(initialDataNoRewrite)) {
	if (identical(initialDataNoRewrite[[slide]]$type, "sheet")) {
		...
		currentSlide <- apply(currentSlide, 1:2, translateRForProccess)
		longitudeTemporaryData <- eval(parse(text = initialDataNoRewrite[[slide]]$name))
		longitudeTemporaryData <- compareMissingValues(longitudeTemporaryData, currentSlide)
...
for (slide in 1:length(initialDataNoRewrite)) {
  if (identical(initialDataNoRewrite[[slide]]$type, "chart")) {
    ...
    regressionParsed <- regression(initialDataNoRewrite[[slide]]$regression)

    if (!identical(regressionParsed, '#ERROR!')) {
      initialDataNoRewrite[[slide]]$data <- saveBackToJsonFromRegression(regressionParsed, initialDataNoRewrite[[slide]]$data)
    }
  }
}
