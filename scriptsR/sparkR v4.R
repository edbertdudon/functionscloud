...
plot <- function(chartdata) {
  ...
  longitudeTemporaryData <- eval(parse(text = name))
  
  if (hasArg(variabley)) {
    pngChart <- ggplot(longitudeTemporaryData, aes(eval(parse(text = variablex)), eval(parse(text = variabley)))) + eval(parse(text = type))
  } else {
    pngChart <-ggplot(longitudeTemporaryData, aes(eval(parse(text = variablex)))) + eval(parse(text = type))
  }

  ggsave("/tmp/ggplot.png", pngChart)
  return(base64enc::dataURI("/tmp/ggplot.png", mime="image/jpeg"))
}
